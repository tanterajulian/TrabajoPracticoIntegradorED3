...
void config_timer3(void){
	...
	//modo match, timer mode, el valor de match se define segun la distancia, no interrumpe,se resetea y no se detiene al dar match
...
void config_timer0(void){
	//modo match, timer mode,  cada 10ms, interrumpe, se resetea y no se detiene al dar match
	TIM_TIMERCFG_Type timer_config;
	TIM_MATCHCFG_Type match_config;
...
void config_timer1(void){
	//:modo match, timer mode, cada 10us, interrumpe, se resetea y se detiene al dar match
	TIM_TIMERCFG_Type timer_config;
	TIM_MATCHCFG_Type match_config;
...
void config_timer2(void){
	//modo match, timer mode, solo se utiliza para contar el tiempo
	TIM_TIMERCFG_Type timer_config;
	TIM_MATCHCFG_Type match_config;
...
void act_timer(){
	TIM_Cmd(LPC_TIM3, DISABLE); //deshabilito el timer
	TIM_ResetCounter(LPC_TIM3);
	if(nivel_actual==0){
...
void conf_uart(void){
	//config por defecto, 9600,8bits, sin paridad, 1 bit stop
